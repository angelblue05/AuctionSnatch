...
function ASinitialize()
   ...
   AS.item={}
	if ASsavedtable then
		if(ASsavedtable[serverName]) then
			 ...
			 --what a pain.  you can't save booleans.  They come back as nil      !!!!!!!!!!!!!!!  @^@#$^!%  swearswear
			 if(ASsavedtable[serverName].ASnodoorbell) then
				ASnodoorbell = ASsavedtable[serverName].ASnodoorbell
				ASprint("|c00ff0000 Checkbox found, doorbell=."..ASnodoorbell)
			 else
				ASprint("|c00ff0000 Checkbox NOT found, doorbell.")
			 end
			 if(ASsavedtable[serverName].ASautostart) then
				ASautostart = ASsavedtable[serverName].ASautostart
			 end
			 if(ASsavedtable[serverName].ASignorebid) then
				ASignorebid = ASsavedtable[serverName].ASignorebid
			 end
			 if(ASsavedtable[serverName].ASignorenobuyout) then
 ...
 --the autostart check button, duh
   if not (ASautostart) then
  ...
  if(AS.mainframe) then
    ...
    --the nodoorbell button
   if not (ASnodoorbell) then
      ASprint("|c0055eeaa doorbell not found.")
      ASnodoorbell = TRUE
    end
  if(AS.mainframe) then
	 AS.mainframe.headerframe.nodoorbell:SetChecked(ASnodoorbell)
	 ASprint("|c0055eeaa AS.mainframe.headerframe.nodoorbell:SetChecked(ASnodoorbell)."..tostring(ASnodoorbell))
  else
        ASprint("|c0055eeaa AS.mainframenot found!?")
  end

	if (not ASignorebid) then
		ASignorebid = false
	end
	AS.mainframe.headerframe.ignorebid:SetChecked(ASignorebid)

   if(not ASignorenobuyout) then
		ASignorenobuyout = false
   end
   AS.mainframe.headerframe.ignorenobuyout:SetChecked(ASignorenobuyout)
...
function AS_OnEvent(self,event)
   ...
   elseif (event=="AUCTION_HOUSE_SHOW") then
      ...
      if (ASautostart == TRUE) then
		if(IsShiftKeyDown()) then  --do the opposite if shift is held
